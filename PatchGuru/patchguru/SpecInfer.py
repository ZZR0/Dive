...
import github
import re
from patchguru.utils.Tracker import append_event, Event, init_pr_log_dir
...
def analyze(project: str, pr_nb: int, force: bool = False) -> None:
    ...
    config_dict = {
        ...
        "DIVE_EXEC_BUDGET": Config.DIVE_EXEC_BUDGET,
        "DIVE_TIME_BUDGET_SEC": Config.DIVE_TIME_BUDGET_SEC,
        "DIVE_DEFLAKE_K": Config.DIVE_DEFLAKE_K,
        "DIVE_MAX_CLUSTERS": Config.DIVE_MAX_CLUSTERS,
        "DIVE_NUM_REVIEW": Config.DIVE_NUM_REVIEW,
    }
    ...
    if Config.PHASE2_STRATEGY == "dive":
        append_event(Event(
            level="INFO", pr_nb=pr_nb,
            message="Phase 2 strategy = DIVE (divergence-directed feedback search). Running dive_search...",
            type="GeneralInfo",
            info={"phase2_strategy": "dive"}
        ))
        from patchguru.analysis.DivergenceSearch import dive_search
        specification = dive_search(
            pr_nb= pr_nb,
            force= force,
        ...
        append_event(Event(
            level="INFO", pr_nb=pr_nb,
            message="Phase 2 strategy = baseline (one-shot LLM generalization). Running spec_generalization...",
            type="GeneralInfo",
            info={"phase2_strategy": "baseline"}
        ))
