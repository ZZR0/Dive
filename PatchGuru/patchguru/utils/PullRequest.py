...
import requests
from patchguru.utils.proxy import requests_proxies
...
class PullRequest:
    ...
    def _pr_url_to_patch(self):
        ...
        resp = requests.get(diff_url, proxies=requests_proxies(), timeout=120)
        resp.raise_for_status()
        resp.encoding = resp.apparent_encoding or "utf-8"
        self.patch = PatchSet(resp.text.splitlines(keepends=True))
    ...
    def count_non_comment_change(self):
        ...
        for modified_file in self.non_test_modified_python_files:
            with open(f"{pre_commit_cloned_repo.repo.working_dir}/{modified_file}", "r") as f:
                old_contents[modified_file] = f.read()
        ...
        for modified_file in self.non_test_modified_python_files:
            ...
            if not equal_modulo_docstrings(old_contents[modified_file], new_file_content):
