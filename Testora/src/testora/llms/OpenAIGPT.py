...
if model_version.startswith("gpt"):
    ...
    base_url = None
    if os.path.exists(".openai_base_url"):
        with open(".openai_base_url", "r") as f:
            base_url = f.read().strip() or None
    openai = OpenAI(api_key=openai_key, base_url=base_url) if base_url else OpenAI(api_key=openai_key)
