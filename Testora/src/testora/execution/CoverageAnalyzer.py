...
def summarize_coverage(pr, test_execution, is_old_version):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as tmp:
        tmp.write(test_execution.coverage_report)
        tmp_coverage_file = tmp.name
    try:
    ...
    for target_file in target_files:
        ...
        try:
            ...
            # the coverage db may lack line data (e.g. scipy's conda env / C
            # extensions where --include paths don't match the imported module),
            # leaving the sqlite file without a "line_bits" table. Coverage is
            # only auxiliary info, so treat it as uncovered instead of crashing.
            covered_lines = None
